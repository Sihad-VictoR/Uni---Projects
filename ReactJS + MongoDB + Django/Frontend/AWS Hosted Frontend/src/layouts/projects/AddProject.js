...
export class AddProject extends Component {
  ...
  project = (event) => {
    ...
    for (let i = 1; i < this.state.selectedEmployeeArray.length; i++) {
      ...
      this.state.employeeDetails.forEach((employee) => {
        ...
        if (employee.id === employee_int) {
          ...
          axios.put("http://18.222.231.105/employee/assign/project/" + employee.id + "/", employee, config)
            .then((response) => {
    ...
    console.log(project);
    axios
      .post("http://18.222.231.105/project/create/", project, config)
      .then((response) => {
        console.log(response.data);
        console.log("abs");
  ...
  updateProject(id) {
    ...
    axios
      .put("http://18.222.231.105/project/update/" + id + "/", project, config)
      .then((response) => {
        if (response.data != null) {
          console.log(response.data);
        }
